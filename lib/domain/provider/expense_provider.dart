...
class ExpenseProvider with ChangeNotifier {
  ...
  List<Expense> get weeklyExpenses {
    ...
    final endOfWeek = startOfWeek.add(Duration(days: 6)).toLocal();

    return _expenseBox.values.where((expense) {
      final expenseDate = expense.date.toLocal();
      return expenseDate.isAfter(startOfWeek.subtract(Duration(days: 1))) && expenseDate.isBefore(endOfWeek.add(Duration(days: 1)));
    }).toList();
  }
